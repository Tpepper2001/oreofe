...
import {
  ...
  Square, CreditCard, ChevronRight, QrCode
} from 'lucide-react';
...
export default function App() {
  ...
  if (!auth) return <LoginScreen onLogin={handleLogin} loading={loading} theme={theme} colors={colors} />;
    ...
    <div style={{ ...styles.app, background: colors.bg, color: colors.text }}>
      <div className="no-print">
        ...
        <main style={styles.main}>
          {loading ? <div style={{textAlign:'center', padding: 50}}>Syncing...</div> : (
            <>
              {auth.role === 'admin' ? 
      ...
      {bulkPrintList.length > 0 && (
        <div className="print-area">
          <div className="print-grid">
            {bulkPrintList.map(m => (
              <div key={m.id} className="print-card">
                ...
                <p>{mode.toUpperCase()} ID CARD</p>
                <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${m.registration_no}`} style={{ width: 140 }} alt="qr" />
          ...
          <button className="no-print" onClick={() => window.print()} style={{ position:'fixed', bottom: 20, right: 20, ...styles.btnPrimary, background: '#10b981' }}>Print All</button>
          <button className="no-print" onClick={() => setBulkPrintList([])} style={{ position:'fixed', bottom: 20, left: 20, ...styles.btnSecondary }}>Close</button>
        </div>
      )}
      ...
      <div style={{ position: 'fixed', top: 20, left: '50%', transform: 'translateX(-50%)', zIndex: 9999 }}>
        {toasts.map(t => (
          <div key={t.id} style={{ padding: '12px 24px', borderRadius: 12, background: t.type === 'error' ? '#ef4444' : '#10b981', color: '#fff', marginBottom: 10, fontWeight: 'bold', animation: 'fadeIn 0.3s ease' }}>{t.message}</div>
        ))}
      </div>
    </div>
  );
...
const ScannerView = ({ profile, onRefresh, showToast, colors, primary, mode }) => {
  ...
  const lookupMember = async (id) => {
    ...
    const { data: m } = await supabase.from(table).select('*').or(`registration_no.eq.${id.toUpperCase()},id.eq.${id}`).maybeSingle();
    if (m) {
  ...
  if (member) return (
    <div style={{ ...styles.card, background: colors.card, borderColor: colors.border, textAlign: 'center', paddingTop: 40, animation: 'fadeIn 0.3s ease' }}>
      ...
      <button onClick={handleTransaction} disabled={processing} style={{ ...styles.btnPrimary, background: primary, width: '100%', height: 60 }}>{processing ? "Processing..." : "Confirm Collection"}</button>
    </div>
  );

  if (active) return (
    <div style={styles.scanContainer}>
      <div style={{ padding: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(10px)' }}>
        <h3 style={{ color: '#fff', margin: 0 }}>Scan Member QR</h3>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        <Scanner onScan={(res) => res?.[0] && lookupMember(res[0].rawValue)} styles={{ container: { width: '100%', height: '100%' } }} components={{ finder: false, audio: false }} />
  ...
  return (
    <div style={{ animation: 'fadeIn 0.4s ease' }}>
      <div style={{ ...styles.card, background: colors.card, borderColor: colors.border, padding: '60px 20px', textAlign: 'center' }}>
        ...
        <h2 style={{ margin: '0 0 10px 0' }}>Ready to Scan</h2>
        <p style={{ opacity: 0.6, marginBottom: 30 }}>Open scanner to process collection</p>
        <button onClick={() => setActive(true)} style={{ ...styles.btnPrimary, background: primary, width: '100%', height: 60, fontSize: 18 }}><QrCode/> Begin Scanning</button>
      </div>
...
const AgentPortal = ({ view, profile, data, onRefresh, showToast, colors, primary, mode }) => {
  if (view === 'dashboard') {
    ...
    const todayTotal = myTs.reduce((s, t) => s + (t.amount || 0), 0);
    return (
      <div style={{ animation: 'fadeIn 0.4s ease' }}>
        <div style={{ ...styles.card, background: primary, color: '#fff', textAlign: 'center', padding: 40, border: 'none' }}>
          <small style={{ opacity: 0.8 }}>COLLECTED TODAY</small>
          <h1 style={{ fontSize: 42, margin: '10px 0' }}>₦{todayTotal.toLocaleString()}</h1>
          <div style={{ background: 'rgba(0,0,0,0.1)', display: 'inline-block', padding: '4px 12px', borderRadius: 20 }}>{myTs.length} Collections</div>
        </div>
...
const MemberManager = ({ data, onRefresh, showToast, colors, primary, mode, setBulkPrintList, isAdmin }) => {
  ...
  return (
    <div>
      ...
      {isAdmin && <button onClick={() => setForm({ show: true, m: null })} style={{ ...styles.btnPrimary, background: primary, width: '100%', marginBottom: 15 }}><UserPlus size={18}/> New Member</button>}
      {selected.length > 0 && (
        ...
        <div style={styles.overlay}>
          <div style={{ ...styles.modal, background: colors.card }}>
            ...
            <form onSubmit={async (e) => {
              ...
              <input name="r" defaultValue={form.m?.registration_no} placeholder="Reg No" style={{ ...styles.input, borderColor: colors.border, marginBottom: 10 }} required />
              <input name="a" type="number" defaultValue={form.m?.expected_amount} placeholder="Amount" style={{ ...styles.input, borderColor: colors.border, marginBottom: 20 }} required />
...
const AgentManager = ({ agents, onRefresh, showToast, colors, primary }) => {
  ...
  return (
    <div>
      <button onClick={() => setForm({ show: true, a: null })} style={{ ...styles.btnPrimary, background: primary, width: '100%', marginBottom: 15 }}><UserPlus size={18}/> Add Agent</button>
      {agents.map(a => (
        <div key={a.id} style={{ ...styles.listItem, background: colors.card, borderColor: colors.border }}>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <button onClick={async () => { if (window.confirm("Delete?")) await supabase.from('employees').delete().eq('id', a.id); onRefresh(); }} style={{ background: 'none', border: 'none', color: '#ef4444' }}><Trash2 size={18}/></button>
          </div>
        </div>
      ))}
      {form.show && (
        <div style={styles.overlay}>
          <div style={{ ...styles.modal, background: colors.card }}>
            ...
            <form onSubmit={async (e) => {
              ...
              <input name="n" defaultValue={form.a?.full_name} placeholder="Name" style={{ ...styles.input, borderColor: colors.border, marginBottom: 10 }} required />
              <input name="e" defaultValue={form.a?.employee_id_number} placeholder="ID" style={{ ...styles.input, borderColor: colors.border, marginBottom: 10 }} required />
  ...
  <div style={{ background: colors.bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
    <div style={{ ...styles.modal, background: colors.card, border: `1px solid ${colors.border}`, textAlign: 'center' }}>
      ...
      <form onSubmit={e => { e.preventDefault(); onLogin({ username: e.target.u.value, password: e.target.p.value }); }}>
        <input name="u" placeholder="ID Number" style={{ ...styles.input, borderColor: colors.border, marginBottom: 10 }} required />
        <input name="p" type="password" placeholder="Password" style={{ ...styles.input, borderColor: colors.border, marginBottom: 20 }} required />
        <button type="submit" disabled={loading} style={{ ...styles.btnPrimary, background: '#3b82f6', width: '100%' }}>{loading ? "Syncing..." : "Login"}</button>
      </form>
    </div>
  </div>
);
...
const ModeSelection = ({ setMode, colors, onLogout }) => (
  <div style={{ background: colors.bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
    <div style={{ width: '100%', maxWidth: 400, textAlign: 'center' }}>
      ...
      <button onClick={onLogout} style={{ marginTop: 40, background: 'none', border: 'none', color: colors.textSecondary, cursor: 'pointer' }}><LogOut size={18}/> Logout</button>
    </div>
  </div>
);