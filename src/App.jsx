            ...
            <div key={m.id} onClick={(e) => handleClick(e, m, idx)} style={{ ...styles.listItem, background: isFocused ? `${colors.primary}15` : colors.card, borderColor: isSelected ? colors.primary : isFocused ? colors.primary : colors.border, cursor: 'pointer' }}>
              ...
              {isAdmin && (
                <div style={{display:'flex', gap: 5}}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); confirmAction("Delete Member", `Permanently delete ${m.full_name}?`, async () => {
...
const ScannerView = ({ profile, onRefresh, showToast, colors, mode }) => {
  ...
  if (member) return (
    <div style={{ ...styles.modalBox, background: colors.card, margin: '0 auto', maxWidth: 350 }}>
      ...
      <p style={{fontSize: 11, opacity: 0.6}}>Amount to Collect:</p>
      <input type="number" value={amt} onChange={e => setAmt(e.target.value)} style={{...styles.bigInput, color: colors.text, borderBottomColor: colors.primary}} autoFocus onFocus={(e) => e.target.select()} />
...
// REST OF COMPONENTS (LOGIN, MODE, HELPERS)
...
const DashboardStats = ({ stats, memberCount, colors }) => (
  <div style={{display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap: 10, marginBottom: 20}}>
    <div style={{...styles.listItem, padding: 10, flexDirection:'column'}}><small style={{opacity:0.6, fontSize:10}}>Today</small><strong>₦{useCountUp(stats.todayRev).toLocaleString()}</strong></div>
    <div style={{...styles.listItem, padding: 10, flexDirection:'column'}}><small style={{opacity:0.6, fontSize:10}}>People</small><strong>{memberCount}</strong></div>
    <div style={{...styles.listItem, padding: 10, flexDirection:'column'}}><small style={{opacity:0.6, fontSize:10}}>Gross</small><strong>₦{useCountUp(stats.totalRev).toLocaleString()}</strong></div>
  </div>
);
    ...
    <div style={styles.overlay}>
      <div style={{ ...styles.modalBox, background: colors.card, maxWidth: 400 }}>
        ...
        <form onSubmit={handleSubmit} style={{ textAlign: 'left', display: 'grid', gap: 10 }}>
          ...
          <input name="am" type="number" defaultValue={member?.expected_amount} placeholder="Daily/Instalment" style={styles.input} required />
          {mode === 'loans' && (
            <>
  ...
  <div style={styles.overlay} className="no-print">
    <div style={{ ...styles.modalBox, background: colors.card, maxWidth: 400 }}>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={() => { setTimeout(() => window.print(), 500); }} style={{ ...styles.btnPrimary, background: colors.primary, flex: 1 }}>Print</button>
