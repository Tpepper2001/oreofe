...
const styles = {
  ...
  bigInput: { fontSize: 40, width: '100%', textAlign: 'center', background: 'none', border: 'none', borderBottom: '3px solid', outline: 'none', marginBottom: 20 },
  ...
  input: { width: '100%', padding: 12, borderRadius: 10, border: '1px solid', background: 'none', color: 'inherit', marginBottom: 10, boxSizing: 'border-box', outline: 'none' },
  ...
  toast: { padding: '12px 20px', borderRadius: 10, color: '#fff', fontWeight: 'bold', minWidth: 200, boxShadow: '0 4px 12px rgba(0,0,0,0.3)' }
};
...
export default function App() {
  ...
  if (!auth) return (
    <LoginScreen 
      onLogin={(c) => {
        if (c.username === CONFIG.admin.username && c.password === CONFIG.admin.password) {
          setAuth({ role: 'admin' });
        } else {
    ...
    <div style={{ ...styles.app, background: colors.bg, color: colors.text }}>
      <div className="no-print">
        ...
        <Navigation 
          ...
          role={auth.role} 
          onNavigate={setView} 
          onLogout={() => confirmAction("Logout", "Exit?", () => setAuth(null))} 
          colors={colors} 
        />
      </div>
      ...
      {bulkPrintList.length > 0 && (
        <BulkPrintConfig 
          members={bulkPrintList} 
          perPage={perPage} 
          setPerPage={setPerPage} 
          onClose={() => setBulkPrintList([])} 
          colors={colors} 
        />
      )}
      <style>
  ...
  return (
    <div style={{ ...styles.loginPage, background: colors.bg }}>
      <div style={{ ...styles.loginCard, background: colors.card, borderColor: colors.border }}>
        ...
        <h2 style={{ color: colors.text, marginBottom: 10 }}>{CONFIG.business.name}</h2>
        <div style={{ display: 'flex', gap: 5, background: theme === 'dark' ? '#020617' : '#f1f5f9', padding: 4, borderRadius: 10, margin: '20px 0' }}>
          <button 
            onClick={() => setType('admin')} 
            style={{ 
              ...styles.tab, 
              background: type === 'admin' ? '#3b82f6' : 'transparent', 
              color: type === 'admin' ? '#fff' : colors.textSecondary 
            }}
          >
            Admin
          </button>
          <button 
            ...
            style={{ 
              ...styles.tab, 
              background: type === 'agent' ? '#3b82f6' : 'transparent', 
              color: type === 'agent' ? '#fff' : colors.textSecondary 
            }}
          >
            Agent
          </button>
...
const ScannerView = ({ profile, onRefresh, showToast, colors, mode }) => {
  ...
  const handleScan = async (res) => {
    ...
    try {
      ...
      let lookup;
      try { 
        lookup = JSON.parse(val).id; 
      } catch (e) { 
        lookup = val; 
      }
      
      const { data: m } = await supabase
        .from(CONFIG.modes[mode].membersTable)
        .select('*')
        .or(`id.eq.${lookup},registration_no.eq.${lookup}`)
        .maybeSingle();
      ...
      showToast("Error reading card", "error"); 
    }
  };
  ...
  if (member) {
    return (
      <div style={{ ...styles.modalBox, background: colors.card, margin: '0 auto', maxWidth: 350 }}>
        ...
        <h2 style={{margin: '10px 0', color: colors.text}}>{member.full_name}</h2>
        <input 
          type="number" 
          value={amt} 
        ...
        <button 
          onClick={async () => {
            const { error } = await supabase
              .from(CONFIG.modes[mode].transTable)
              .insert([{ 
...
const MemberManagement = ({ members, transactions, onRefresh, showToast, colors, isAdmin, mode, confirmAction, setBulkPrintList }) => {
  ...
  const filtered = useMemo(() => 
    members.filter(m => 
      ...
      (m.registration_no || '').includes(search.toUpperCase())
    )
  , [members, search]);
  ...
  return (
    <div onKeyDown={handleKeyboard} tabIndex={0} style={{outline:'none'}}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 15 }}>
        {isAdmin && (
          <button 
            onClick={() => setForm({ show: true, member: null })} 
            style={{ 
              ...styles.btnPrimary, 
              background: colors.primary, 
              flex: 1 
        ...
        {isAdmin && (
          <div style={{display:'flex', gap:5}}>
            <button 
              ...
              style={{...styles.smallGhostBtn, color: colors.text, borderColor: colors.border}}
            >
              All
            </button>
            <button 
      ...
      {selectedIds.length > 0 && (
        <div style={{
          ...
          <div style={{display:'flex', gap: 8}}>
            <button 
              onClick={() => setBulkPrintList(members.filter(m => selectedIds.includes(m.id)))} 
              style={{
                ...styles.btnPrimary, 
                padding: '8px 16px', 
                fontSize: 12, 
      ...
      {form.show && (
        <MemberForm 
          ...
          onSuccess={() => { 
            setForm({ show: false, member: null }); 
            onRefresh(); 
          }} 
          showToast={showToast} 
          colors={colors} 
        ...
        {filtered.map((m, idx) => {
          ...
          return (
            <div 
              ...
              style={{ 
                ...styles.listItem, 
                background: isFocused ? `${colors.primary}11` : colors.card, 
              ...
              {isAdmin && (
                <div style={{marginRight: 12}}>
                  {isSelected ? (
                    ...
                    <Square size={20} opacity={0.2} color={colors.text}/>
                  )}
                </div>
              )}
              <div style={{ flex: 1 }}>
                <strong>{m.full_name}</strong>
                <div style={{...styles.subtext, color: colors.textSecondary}}>
                  {m.registration_no}
                </div>
              </div>
              {isAdmin && (
    ...
    <div style={{ textAlign: 'center', width: '100%', maxWidth: 380 }}>
      ...
      <div style={{ display: 'grid', gap: 15 }}>
        <button 
          ...
          style={{ 
            ...styles.btnPrimary, 
            border: `2px solid ${CONFIG.modes.ajo.primary}`, 
            color: CONFIG.modes.ajo.primary, 
        ...
        <button 
          ...
          style={{ 
            ...styles.btnPrimary, 
            border: `2px solid ${CONFIG.modes.loans.primary}`, 
            color: CONFIG.modes.loans.primary, 
            background: 'transparent', 
            height: 100, 
            fontSize: 18 
          }}
...
const Header = ({ business, role, isDark, onToggleTheme, onSwitchMode, colors }) => (
  <header style={{ 
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      <h1 style={{fontSize: 15, fontWeight: '900', color: colors.text}}>{business}</h1>
    </div>
    <button 
...
const AdminPortal = ({ view, data, onRefresh, showToast, colors, mode, setBulkPrintList, confirmAction }) => {
  const stats = useMemo(() => {
    ...
    return { 
      todayRev: data.transactions
        .filter(t => t.created_at.startsWith(today))
        .reduce((s, t) => s + (t.amount || 0), 0), 
      totalRev: data.transactions
        .reduce((s, t) => s + (t.amount || 0), 0) 
    };
  }, [data.transactions]);
    ...
    return (
      <div style={styles.fadeIn}>
        <div style={styles.statsGrid}>
          <StatCard 
            title="Today" 
            value={`₦${useCountUp(stats.todayRev).toLocaleString()}`} 
            colors={colors} 
          />
          <StatCard 
            title="People" 
          ...
          <StatCard 
            title="Total" 
            value={`₦${useCountUp(stats.totalRev).toLocaleString()}`} 
            colors={colors} 
          />
        </div>
        <SectionHeader title="Recent Activity" icon={<TrendingUp size={20} />} colors={colors} />
        <TransactionList 
    ...
    return (
      <div style={styles.fadeIn}>
        <div style={{
          ...
          <h1 style={{fontSize: 32, margin: '10px 0'}}>
            ₦{useCountUp(stats.todayTotal).toLocaleString()}
          </h1>
...
const MemberForm = ({ member, mode, onClose, onSuccess, showToast, colors }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { 
      ...
      phone_number: fd.get('p') || '', 
      address: fd.get('a') || '', 
      expected_amount: Number(fd.get('am')), 
      ajo_owner_id: 'admin' 
    };
  ...
  return (
    <div style={styles.overlay}>
      <div style={{ 
        ...
        <h3 style={{color: colors.text}}>{isEdit ? 'Update' : 'New'} Member</h3>
        <form 
        ...
        >
          ...
          <input 
            name="r" 
            defaultValue={member?.registration_no} 
          ...
          <input 
            name="am" 
            type="number" 
            defaultValue={member?.expected_amount} 
            placeholder="Amount" 
            style={{...styles.input, borderColor: colors.border, color: colors.text}} 
            required 
          />
          {mode === 'loans' && (
            <>
              <input 
                name="tla" 
                type="number" 
                defaultValue={member?.total_loan_amount} 
              ...
              <input 
                name="ttr" 
                type="number" 
                defaultValue={member?.total_to_repay} 
...
const AgentForm = ({ agent, onClose, onSuccess, showToast, colors }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = { 
      full_name: fd.get('n'), 
      employee_id_number: fd.get('id').toLowerCase(), 
      password: fd.get('p') 
    };
  ...
  return (
    <div style={styles.overlay}>
      <div style={{ 
        ...
        <h3 style={{color: colors.text}}>{isEdit ? 'Edit' : 'New'} Agent</h3>
        <form 
          onSubmit={handleSubmit} 
          style={{ 
            textAlign: 'left', 
        ...
        >
          ...
          <input 
            name="id" 
            defaultValue={agent?.employee_id_number} 
...
const AgentManagement = ({ agents, onRefresh, showToast, colors, confirmAction }) => {
  ...
  return (
    <div>
      <button 
        onClick={() => setForm({ show: true, agent: null })} 
        style={{ 
          ...styles.btnPrimary, 
          background: colors.primary, 
          marginBottom: 15, 
      ...
      <div style={styles.list}>
        {agents.map(a => (
          ...
          >
            <div style={{ flex: 1 }}>
              <strong style={{color: colors.text}}>{a.full_name}</strong>
              <br/>
              <small style={{...styles.subtext, color: colors.textSecondary}}>
                ID: {a.employee_id_number}
              </small>
            </div>
            <div style={{display:'flex', gap:5}}>
              <button 
                onClick={() => setForm({ show: true, agent: a })} 
                style={{ ...styles.iconBtn, color: colors.primary }}
              >
                <Edit3 size={18} />
              </button>
...
const BulkPrintConfig = ({ members, perPage, setPerPage, onClose, colors }) => (
  <div style={styles.overlay} className="no-print">
    <div style={{ 
      ...
      <h3 style={{color: colors.text}}>Print {members.length} Cards</h3>
      <div style={{ 
        display: 'grid', 
        ...
        {[1, 4, 8, 12].map(n => (
          <button 
            ...
            style={{ 
              ...
              borderRadius: 10, 
              border: '1px solid', 
              background: perPage === n ? colors.primary : 'transparent', 
              color: perPage === n ? '#fff' : colors.text, 
              borderColor: colors.primary,
              cursor: 'pointer'
            }}
          >
...
const StatCard = ({ title, value, colors }) => (
  <div style={{ 
    ...
    <small style={{ opacity: 0.6, fontSize: 10, color: colors.textSecondary }}>
      {title}
    </small>
...
const SearchBar = ({ value, onChange, placeholder, colors }) => (
  <div style={{ 
    ...
    <Search size={18} opacity={0.5} color={colors.textSecondary} />
    <input 
      value={value} 
...
const TransactionList = ({ transactions, colors }) => (
  <div style={styles.list}>
    {transactions.map(t => (
      ...
      >
        <div style={{ flex: 1 }}>
          <strong style={{color: colors.text}}>{t.full_name}</strong>
          <br/>
          <small style={{...styles.subtext, color: colors.textSecondary}}>
            {t.employee_name || 'Admin'}
          </small>
        </div>
...
const ConfirmationModal = ({ modal, onClose, colors }) => (
  <div style={styles.overlay}>
    <div style={{ 
      ...
      <h3 style={{color: colors.text}}>{modal.title}</h3>
      <p style={{fontSize: 14, margin: '15px 0', color: colors.text}}>
        {modal.msg}
      </p>
      <div style={{ display: 'flex', gap: 10 }}>
        <button 
