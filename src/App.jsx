    ...
    <div style={{ ...styles.loginPage, background: colors.bg }}>
      <div style={{ ...styles.loginCard, background: colors.card, borderColor: colors.border }}>
        ...
        <form onSubmit={e => { e.preventDefault(); onLogin({ username: e.target.u.value, password: e.target.p.value }); }}>
          <input name="u" placeholder={type === 'admin' ? "Admin Username" : "Agent ID"} style={styles.loginInput} required />
...
const ScannerView = ({ profile, onRefresh, showToast, colors, mode }) => {
  ...
  const handleScan = async (res) => {
    ...
    try {
      let lookup;
      try { lookup = JSON.parse(rawValue).id; } catch (e) { lookup = rawValue; }
  ...
  if (member) return (
    <div style={{ ...styles.modalBox, background: colors.card, margin: '0 auto', maxWidth: 400 }}>
      <div style={{marginBottom: 20}}>
        <div style={{...styles.avatar, background: colors.primary + '22', color: colors.primary, width: 60, height: 60, margin: '0 auto 10px', fontSize: 24}}>{member.full_name[0]}</div>
        <small style={{color: colors.primary, fontWeight:'bold'}}>{member.registration_no}</small>
        <h2 style={{margin: '5px 0'}}>{member.full_name}</h2>
      </div>

      <div style={{textAlign:'left', marginBottom: 15}}>
        <label style={{fontSize: 12, opacity: 0.6, marginLeft: 5}}>Confirm Amount</label>
      ...
      <div style={{display:'flex', gap: 10}}>
        ...
        }} style={{ ...styles.btnPrimary, background: colors.primary, flex: 2 }}>Save Collection</button>
        <button onClick={() => setMember(null)} style={{ ...styles.btnSecondary, flex: 1 }}>Cancel</button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ textAlign: 'center', padding: '20px 0' }}>
      {!scanning ? (
        <button onClick={() => setScanning(true)} style={{...styles.scanBtn, borderColor: colors.primary, color: colors.primary, background: colors.primary + '11'}}>
          <Camera size={48} style={{marginBottom: 10}}/><br/>Open Scanner
        </button>
      ) : (
        <div style={{ position: 'relative', borderRadius: 24, overflow: 'hidden', border: `4px solid ${colors.primary}` }}>
          <Scanner onScan={handleScan} allowMultiple={false} scanDelay={2000} />
          <button onClick={() => setScanning(false)} style={{ position:'absolute', top: 15, right: 15, background: '#ef4444', color: '#fff', borderRadius: '50%', padding: 10, border:'none', zIndex: 10 }}><X size={24}/></button>
          <div style={{position:'absolute', bottom: 20, left: 0, right: 0, color: '#fff', textShadow: '0 2px 4px rgba(0,0,0,0.5)', fontWeight: 'bold'}}>Align QR code within frame</div>
        </div>
      )}
    </div>
  );
...
/* ===================== MEMBER MANAGEMENT (KEYBOARD + SHIFT) ===================== */
    ...
    <div style={styles.fadeIn} onKeyDown={handleKeyboard} tabIndex={0} style={{outline:'none'}}>
      ...
      <div style={styles.list}>
        {filtered.map((m, idx) => {
          ...
          return (
            <div key={m.id} onClick={(e) => handleClick(e, m, idx)} style={{ ...styles.listItem, background: isFocused ? `${colors.primary}11` : colors.card, borderColor: isSelected ? colors.primary : isFocused ? colors.primary : colors.border, cursor: 'pointer', userSelect: 'none' }}>
...
const styles = {
  ...
  scanBtn: { padding: '50px 20px', borderRadius: 24, width: '100%', border: '2px dashed', background: 'none', fontWeight: 'bold', fontSize: 18, cursor: 'pointer' },
  ...
  avatar: { width: 40, height: 40, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' },
    ...
    <div style={styles.overlay}>
      <div style={{ ...styles.modalBox, background: colors.card, maxWidth: 400 }}>
        ...
        <form onSubmit={handleSubmit} style={{ textAlign: 'left', display: 'grid', gap: 10 }}>
          ...
          <input name="r" defaultValue={member?.registration_no} placeholder="Serial (e.g AJO001)" style={styles.input} required />
          <input name="am" type="number" defaultValue={member?.expected_amount} placeholder="Amount" style={styles.input} required />
          {mode === 'loans' && (
            <><input name="tla" type="number" defaultValue={member?.total_loan_amount} placeholder="Loan Taken" style={styles.input} required /><input name="ttr" type="number" defaultValue={member?.total_to_repay} placeholder="Total Back" style={styles.input} required /></>
          )}
